...
v528=`$PERL -e'print (($] < 5.028)?0:1)'`
v530=`$PERL -e'print (($] < 5.030)?0:1)'`
...
# require $fname
tests[28]='my($fname,$tmp_fh);while(!open($tmp_fh,">",($fname=q{ccode28_} . rand(999999999999)))){$bail++;die "Failed to create a tmp file after 500 tries" if $bail>500;}print {$tmp_fh} q{$x="ok";1;};close($tmp_fh);sleep 1;push @INC, "."; require $fname;END{unlink($fname);};print $x;'
...
# these errors are at compile time as expected - need to check STDERR when compiling
#stderr[173]='Prototype mismatch: sub main::BEGIN () vs none at ./ccode173.pl line 2.
...
#issue 30
tests[230]='sub f1 { my($self) = @_; $self->f2;} sub f2 {} sub new {} print "ok|@ARGV|\n";'
result[230]='ok||'
...
result[261]='2015'
tests[262]='use POSIX; print q[ok]'
result[262]='ok'
...
tests[2052]='use utf8; { package Diӑmond_A; sub fಓ { "ok" } } { package Diӑmond_B; use base q{Diӑmond_A}; use mro "c3"; sub fಓ { (shift)->next::method() } } print Diӑmond_B->fಓ();'
