...
is(
    latin1_to_native("\U\x{DF}aB\x{149}cD"), latin1_to_native("SSAB\x{2BC}NCD"),
...
is(
    latin1_to_native("\L\x{DF}aB\x{149}cD"), latin1_to_native("\x{DF}ab\x{149}cd"),
...
is(
    latin1_to_native("\F\x{DF}aB\x{149}cD"), latin1_to_native("ssab\x{2BC}ncd"),
...
for ( 1, 4, 9, 16, 25 ) {
    is(
        uc "\x{03B0}" x $_, "\x{3a5}\x{308}\x{301}" x $_,
    ...
    is(
        fc "\x{03B0}" x $_, "\x{3C5}\x{308}\x{301}" x $_,
...
like uc delete $h{k}, qr "^(?:\x{3a5}\x{308}\x{301}){3}BCDE=ARRAY\(.*\)", 'uc(TEMP ref) does not produce a corrupt string';
$h{k} = bless [], "\x{130}bcde";                  # U+0130 grows with lc()
                                                  # using delete marks it as TEMP, so uc-in-place is permitted
like lc delete $h{k}, qr "^i\x{307}bcde=array\(.*\)", 'lc(TEMP ref) does not produce a corrupt string';
